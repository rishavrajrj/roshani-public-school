...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Passcode quick fallback check
    if (emailVal === 'admin2026' || emailVal === 'rps2026' || emailVal === 'admin') {
      if (typeof window !== 'undefined') {
        sessionStorage.setItem('rps_admin_session', 'true');
        localStorage.setItem('rps_admin_session', 'true');
    ...
    try {
      if (typeof window !== 'undefined' && (window as any).RPS_Supabase) {
        await (window as any).RPS_Supabase.loginAdmin(emailVal, password);
        sessionStorage.setItem('rps_admin_session', 'true');
        localStorage.setItem('rps_admin_session', 'true');
        window.location.href = '/admin';
      } else {
  ...
  const handleForgotPassword = (e: React.MouseEvent) => {
    e.preventDefault();
    if (typeof window !== 'undefined') {
      const emailAddr = prompt('Enter your admin email address to receive password reset instructions:');
      if (emailAddr) {
        alert('If this email is registered in Supabase Auth, a reset link has been dispatched.');
      }
    }
  };
      ...
      <div className="relative z-10 min-h-screen flex flex-col justify-between">
        ...
        <header className="w-full flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3.5">
            <img
              ...
              className="w-[100px] h-[100px] object-contain drop-shadow-[0_4px_14px_rgba(0,0,0,0.85)]"
            />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 lg:gap-12 items-center my-auto py-5">
          ...
          <section className="w-full max-w-[360px] bg-white rounded-xl overflow-hidden shadow-2xl shadow-black/60 justify-self-center lg:justify-self-end order-1 lg:order-2">
            ...
            {/* Form */}
            <form id="login-form" onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-3.5 text-slate-800">
              {error && (
                <div id="error-box" className="p-2.5 bg-red-50 border-l-4 border-red-600 text-red-800 text-xs rounded font-medium flex items-center gap-2">
                  <span>{error}</span>
                </div>
              )}
              ...
              <div className="w-full flex items-center justify-between shrink-0 text-xs pt-0.5">
                ...
                <a id="forgot-pass-link" href="#" onClick={handleForgotPassword} className="font-semibold text-[#B91C5C] hover:underline text-[11px] sm:text-xs">
                  Forgot Password?
                </a>
              </div>
